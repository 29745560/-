...
- (void)_animateForChange:(NSInteger)targetIndex{
    ...
    newFrame.origin.x = _selectedTabButton.frame.origin.x+15;
    newFrame.size.width = _selectedTabButton.frame.size.width-30;
    CGRect tranFrame;
...
- (void)layoutUI{
    ...
    if (_tabsContainer.subviews.count > 0) {
        ...
        [_indicatorLine setFrame:CGRectMake(_selectedTabButton.frame.origin.x+15, _selectedTabButton.frame.size.height - _indicatorLineHeight, _selectedTabButton.frame.size.width-30, _indicatorLineHeight)];
    }
